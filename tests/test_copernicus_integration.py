...
from src.data.copernicus.quality import assess_s1_quality, assess_s2_quality
...
class TestCopernicusIntegration(unittest.TestCase):
    ...
    def test_s2_quality_assessment(self):
        """Test S2 quality assessment (cloud cover, data completeness).

        WHAT THIS TESTS:
        - Cloud mask extraction from SCL band
        - Cloud coverage percentage calculation
        - Data completeness check
        - Overall quality score computation

        WHY IT MATTERS:
        Quality assessment helps filter out unusable images before processing.
        High cloud cover can make optical imagery useless for analysis.

        NOTE: This test may skip if the S2 product is Level-1C (no SCL band).
        Level-2A products are required for cloud masking.
        """
        if self.s2_file is None:
            self.skipTest("S2 test data not available")

        quality = assess_s2_quality(self.s2_file)

        # Verify structure
        self.assertIsNotNone(quality, "Quality assessment failed")
        ...
        # Check score range
        self.assertGreaterEqual(quality["overall_score"], 0.0, "Score should be >= 0")
        self.assertLessEqual(quality["overall_score"], 1.0, "Score should be <= 1")

        # Check data completeness
        self.assertGreaterEqual(quality["data_completeness"], 0.0)
        self.assertLessEqual(quality["data_completeness"], 100.0)

        print(
            f"✓ S2 quality: {quality['quality_level']} "
            f"(score={quality['overall_score']:.2f}, "
            f"clouds={quality.get('cloud_coverage', 'N/A')}%)"
        )
        if quality["notes"]:
            print(f"  Notes: {', '.join(quality['notes'])}")
    ...
    def test_s1_quality_assessment(self):
        """Test S1 quality assessment (data completeness, file integrity).

        WHAT THIS TESTS:
        - ZIP file integrity
        - Presence of measurement files (polarizations)
        - Manifest file existence
        - Overall quality score computation

        WHY IT MATTERS:
        SAR data can have acquisition gaps or file corruption issues.
        Quality assessment helps identify problematic products before processing.

        NOTE: SAR doesn't have cloud issues (works through clouds), but can
        have missing bursts or incomplete polarizations.
        """
        if self.s1_file is None:
            self.skipTest("S1 test data not available")

        quality = assess_s1_quality(self.s1_file)
        ...
        print(
            f"✓ S1 quality: {quality['quality_level']} "
            f"(score={quality['overall_score']:.2f}, "
            f"completeness={quality['data_completeness']:.0f}%)"
        )
        if quality["notes"]:
            print(f"  Notes: {', '.join(quality['notes'])}")
