    ...
    def forward(self, x, y):
        ...
        return {
            "cross_entropy_loss": nn.functional.cross_entropy(
                x.flatten(2, 3), y["label"].flatten(1, 2).long(), weight=self.weights
            )
        }
...
class CrossEntropyIgnore(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if len(y["label"].shape) > 1:
            ...
            label = y["label"]
        return {
            "cross_entropy_loss": nn.functional.cross_entropy(x, label.long(), ignore_index=-1)
        }
...
class MILNCE(nn.Module):
    ...
    def forward(self, input, y):
        ...
        diag_mask = torch.block_diag(
            *[torch.ones(n_patches, n_patches) for _ in range(len(logits) // n_patches)]
        )
        ...
        if masks:
            ...
            mask = (
                torch.cat([x[f"masks_{modality}"] for modality in modalities], dim=1)
                .flatten(0, 1)
                .float()
            )
            ...
            loss = torch.logsumexp(
                logits[valid_entries].view(-1, valid_entries.sum(1).max()) / self.tau, dim=1
            ).sum()
        else:
        ...
        idx = torch.tensor(
            [
                [i + j * n_tokens for j in range(len(modalities)) if j != k]
        ...
        return {"contrastive_loss": loss / len(features), "logits": logits}
