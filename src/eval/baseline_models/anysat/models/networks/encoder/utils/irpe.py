...
BUCKET_IDS_BUF = dict()  # type: ignore
...
def get_bucket_ids_2d(
    method, height, width, skip, alpha, beta, gamma, dtype=torch.long, device=torch.device("cpu")
):
...
class iRPE(nn.Module):
    ...
    def _get_rp_bucket(self, x, height=None, width=None, pos=None, modis=False):
        ...
        rp_bucket = torch.cat(
            [rp_bucket[:, :1, :]] + [rp_bucket[:, 1:, :] for _ in range(self.n_modalities)], dim=1
        )
        rp_bucket = torch.cat(
            [rp_bucket[:, :, :1]] + [rp_bucket[:, :, 1:] for _ in range(self.n_modalities)], dim=2
        )
...
def get_rpe_config(
    ratio=1.9, method=METHOD.PRODUCT, mode="contextual", shared_head=True, skip=0, rpe_on="k"
):
