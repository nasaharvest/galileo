...
class Attention(nn.Module):
    fused_attn: Final[bool]  # type: ignore
...
class RPEAttention(nn.Module):
    ...
    def __init__(
        ...
        self.rpe_q, self.rpe_k, self.rpe_v = build_rpe(
            rpe_config, head_dim=head_dim, num_heads=num_heads, n_modalities=n_modalities
        )
...
class CrossRPEAttentionMulti(nn.Module):
    def __init__(
        ...
        if not release:
            ...
            self.modis = {
                dataset: int("modis" in modalities[dataset]) for dataset in self.datasets
            }
            self.len_modalities = {}
            self.pos_embed = {}
            for dataset in self.datasets:
                ...
                for scale in scales[dataset]:
                    ...
                    self.pos_embed[
                        "_".join([dataset, str(scale)])
                    ] = get_2d_sincos_pos_embed_with_scale(
                        dim, int(num_p**0.5), scale, cls_token=True, modis=self.modis[dataset]
                    )
