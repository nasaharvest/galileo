...
class JEPAAny(nn.Module):
    ...
    def mask_modalities(self, x):
        ...
        final_keep = torch.sum(
            torch.nn.functional.one_hot(torch.cat([keep, ids_keep], dim=1), num_classes=L), dim=1
        )
...
class JEPAAnyMulti(nn.Module):
        ...
        for modality in modalities:
            if modality + "_dates" in keys:
                ...
                batch_idx = (
                    ...
                    .unsqueeze(1)
                    .expand(-1, int(n_patches * (1 - ratio)))
                )
    ...
    def forward_decoder(self, out, tokens, mask_enc, mask_pred):
        # embed tokens
        out["predicted_tokens"] = self.predictor(
            tokens[:, 1:, :], mask_enc, mask_pred, out["dataset"], out["scale"]
        )
        return out
