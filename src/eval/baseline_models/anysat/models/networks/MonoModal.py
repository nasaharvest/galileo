...
class MonoModal(nn.Module):
    ...
    def __init__(self, encoder, mlp, modalities: list = [], omni: bool = False):
        super().__init__()
...
class MonoModalMulitScale(nn.Module):
    ...
    def __init__(self, encoder, mlp, modalities: dict = {}, scales: dict = {}, omni: bool = False):
        super().__init__()
        self.encoder = encoder
        self.scales = scales
        self.omni = omni
...
class MonoModalFLAIR(nn.Module):
    ...
    def forward_test(self, x):
        img = x["aerial-flair"]
        logits = torch.zeros((img.shape[0], self.mlp.final_dim, img.shape[2], img.shape[3])).to(
            img.device
        )
        for i in range(2):
            for j in range(2):
