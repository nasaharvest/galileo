...
class MLPSemSeg(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (
            x.view(B, N, 1, D)
            .view(B, N, 1, self.final_dim, self.patch_size * self.patch_size)
            .permute(0, 2, 3, 1, 4)
        )
        x = x.view(B, 1, self.final_dim, N, self.patch_size, self.patch_size)
        x = x.view(
            B, 1, self.final_dim, num_patches, num_patches, self.patch_size, self.patch_size
...
class MLPDenseSemSegFLAIR(nn.Module):
    ...
    def forward(self, features):
        ...
        x = x.view(
            ...
            self.patch_size,
            self.patch_size,
        ).permute(0, 1, 4, 5, 6, 2, 7, 3)
        x = x.reshape(
            B,
            1,
            self.final_dim,
            N,
            self.proj_size * self.patch_size,
            self.patch_size * self.proj_size,
        )
        x = x.view(
            B,
            1,
...
class MLPDenseSemSegFLAIRmulti(nn.Module):
    ...
    def forward(self, features, patch_size):
        ...
        x = x.view(
            B, 1, self.proj_size, self.proj_size, self.final_dim, N, patch_size, patch_size
        ).permute(0, 1, 4, 5, 6, 2, 7, 3)
        x = x.reshape(
            B, 1, self.final_dim, N, self.proj_size * patch_size, patch_size * self.proj_size
        )
        x = x.view(
            B,
            1,
...
class MLPDenseSemSegDeconv(nn.Module):
    def __init__(
        ...
        self.deconv = nn.ModuleList(
            [
                ...
                nn.Conv2d(
                    initial_dim // 2, final_dim, kernel_size=1, stride=1, padding=0, bias=True
                ),
            ]
        )
        self.patch_size = patch_size
    ...
    def forward(self, features):
        ...
        x = x.unsqueeze(2).permute(0, 2, 4, 1, 3)
        x = x.view(B, 1, D, N, self.patch_size, self.patch_size)
        x = x.view(B, 1, D, num_patches, num_patches, self.patch_size, self.patch_size).permute(
            0, 1, 2, 3, 5, 4, 6
        )
...
class MLPDenseSemSegDeconvFLAIR(nn.Module):
    def __init__(
        ...
        self.deconv = nn.ModuleList(
            [
                nn.Conv2d(
                    initial_dim * 2, initial_dim, kernel_size=1, stride=1, padding=0, bias=True
                ),
                BilinearUpsampleLayer(output_size=(125, 125)),
                nn.Conv2d(
                    initial_dim, initial_dim // 2, kernel_size=3, stride=1, padding=1, bias=True
                ),
                ...
                nn.Conv2d(
                    initial_dim // 4, final_dim, kernel_size=1, stride=1, padding=0, bias=True
                ),
            ]
        )
        if resolution < 1:
...
class DecoderDeconvNoIndicesAerial(nn.Module):
    ...
    def __init__(self, in_channels: int = 10, embed_dim: int = 128):
        super(DecoderDeconvNoIndicesAerial, self).__init__()
        self.decode = nn.ModuleList(
            [
                nn.ConvTranspose2d(
                    embed_dim, embed_dim, kernel_size=3, stride=1, padding=0, bias=False
                ),
                nn.ConvTranspose2d(
                    embed_dim, embed_dim, kernel_size=4, stride=1, padding=0, bias=False
                ),
                nn.ConvTranspose2d(
                    embed_dim, embed_dim, kernel_size=4, stride=2, padding=1, bias=False
                ),
                nn.ConvTranspose2d(
                    embed_dim, embed_dim, kernel_size=3, stride=2, padding=0, bias=True
                ),
                nn.ConvTranspose2d(
                    embed_dim, in_channels, kernel_size=3, stride=2, padding=1, bias=True
                ),
            ]
        )
    ...
    def __init__(
        ...
        self.deconv = nn.ModuleList(
            [
                ...
                nn.Conv2d(
                    initial_dim // 4, final_dim, kernel_size=3, stride=1, padding=1, bias=True
                ),
            ]
        )
