    ...
    def forward(self, x):
        ...
        if self.n_class:
            if self.keep_subpatch:
                ...
                x = x.view(
                    B, 1, self.n_class, num_patches, num_patches, self.patch_size, self.patch_size
            ...
            x = (
                x.view(B, N, 1, D)
                .view(B, N, 1, self.n_class, self.patch_size * self.patch_size)
                .permute(0, 2, 3, 1, 4)
            )
            x = x.view(B, 1, self.n_class, N, self.patch_size, self.patch_size)
            x = x.view(
        ...
        for scale in self.scales[x["dataset"]]:
            ...
            if self.global_pool:
                ...
                if self.keep_subpatch:
                    ...
                    out = out.view(
                        B, 1, self.n_class, num_patches, num_patches, scale, scale
                    ).permute(0, 1, 2, 3, 5, 4, 6)
                    ...
                    out = (
                        out.view(B, N, 1, D)
                        .view(B, N, 1, self.n_class, scale * scale)
                        .permute(0, 2, 3, 1, 4)
                    )
                    out = out.view(B, 1, self.n_class, N, scale, scale)
                    out = out.view(
