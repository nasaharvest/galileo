...
class Fine(nn.Module):
            ...
            if name in key:
                if target:
                    if "projector" in key:
                        ...
                        if not (proj_only) and "predictor." not in key:
                            d[".".join(key.split(".")[1:])] = u["state_dict"][key]
                else:
    ...
    def forward(self, x):
        ...
        if "dataset" in keys:
            ...
            out = (
                out.permute(0, 1, 2, 3, 5, 4, 6)
                .reshape(B, 1, self.final_dim, size, size)
                .flatten(0, 1)
            )
            return out
        if self.global_pool:
    ...
    def forward_test(self, x):
        img = x["aerial-flair"]
        logits = torch.zeros((img.shape[0], self.final_dim, img.shape[2], img.shape[3])).to(
            img.device
        )
        for i in range(2):
            for j in range(2):
                ...
                if "dataset" in x.keys():
                    ...
                    for scale in self.scales[x["dataset"]]:
                        ...
                        features = (
                            features[:, 1:]
                            .unsqueeze(2)
                            .repeat(1, 1, out["subpatches"].shape[2], 1)
                        )
                        dense_x = torch.cat([features, out["subpatches"]], dim=3)
                        out = self.head(dense_x)
                        B, N, _, D = out.shape
                    ...
                    features, out = self.model(x)
                    features = (
                        features[:, 1:].unsqueeze(2).repeat(1, 1, out["subpatches"].shape[2], 1)
                    )
                    dense_x = torch.cat([features, out["subpatches"]], dim=3)
                    out = self.head(dense_x)
                    B, N, _, D = out.shape
